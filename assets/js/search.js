(function() {
  ...
  <article class="flex flex-col gap-y-3 p-6 mt-6 mx-2 md:mx-0 rounded-lg shadow-md bg-white dark:bg-neutral-700" id="result-{{= it.index }}">
    <h2 class="text-4xl font-semibold text-neutral-800 dark:text-neutral-200">
      <a href="{{= it.permalink }}">{{! it.title }}</a>
    </h2>
    <h3 class="my-4 text-large text-neutral-600 dark:text-neutral-300">
      {{! it.snippet }}
    </h3>
    <ul class="flex flex-row flex-wrap text-neutral-500 dark:text-neutral-300">
      {{~ it.categories :v }}
      <li>
        <span
          class="text-sm mr-2 px-2 py-1 rounded border border-emerald-800 bg-emerald-800 text-neutral-50">
          {{!v}}
        </span>
      </li>
      {{~}}
